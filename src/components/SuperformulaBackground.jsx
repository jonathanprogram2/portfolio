  ...
  useEffect(() => {
    ...
    class SuperformulaWireframe {
      constructor() {
        ...
        this.themes = {
          Synthwave: {
            colors: ["#3ACBD4", "#ACF6F7", "#ca24aa", "#f868d7"],
            burstColor: "#ffffff",
          },
          Forest: {
      ...
      init() {
        ...
        // Set background image inside WebGL (so it will always show)
        const tex = new THREE.TextureLoader().load(heroBg);
        tex.colorSpace = THREE.SRGBColorSpace; // newer three.js
      ...
      getInterpolatedParams() {
        ...
        if (
          ...
          this.wireframeMesh.material.uniforms.morphProgress.value =
            morphEffect;
        }
      ...
      buildCustomPanel() {
        ...
        if (isMobile) {
          ...
          el.innerHTML = `
                        <div class="sf-sidebar-root">
    <aside class="sf-sidebar-glass">
      <button class="sf-side-icon" data-panel="shape" aria-label="Shape controls">
        ◫
      </button>

      <button class="sf-side-icon" data-panel="anim" aria-label="Animation and color controls">
        ✦
      </button>

      <button class="sf-side-icon" data-panel="bloom" aria-label="Bloom controls">
        ✺
      </button>

      <button class="sf-side-icon energy-button" id="sf-burst" aria-label="Energy burst">
        ✨
      </button>

      <div class="sf-sidebar-tryme">
  <span>T</span>
  <span>R</span>
  <span>Y</span>
  <span class="sf-sidebar-space"></span>
  <span>M</span>
  <span>E</span>
</div>
    </aside>

    <div class="sf-side-popout" id="sf-side-popout">
      <div class="sf-side-panel" data-panel-content="shape">
        <div class="sf-card">
          ...
          <div class="sf-row">
            ...
            <select class="sf-select" id="sf-preset">
              ${Object.keys(this.presetOptions)
                .map(
                  (name) =>
                    `<option value="${this.presetOptions[name]}" ${
                      this.presetOptions[name] === this.params.preset
                        ? "selected"
                        : ""
                    }>${name}</option>`,
                )
          ...
          <div class="sf-row">
            ...
            <div class="sf-range-wrap">
              ...
              <div class="sf-val" id="sf-morph-val">${this.params.morphDuration.toFixed(2)}</div>
            </div>
          </div>
        </div>
      </div>

      <div class="sf-side-panel" data-panel-content="anim">
        <div class="sf-card">
          ...
          <div class="sf-row">
            ...
            <div class="sf-range-wrap">
              <input class="sf-range" id="sf-ps" type="range" min="0" max="3" step="0.05" value="${this.params.pulseSpeed}">
          ...
          <div class="sf-row">
            ...
            <div class="sf-range-wrap">
              <input class="sf-range" id="sf-pi" type="range" min="0" max="1.5" step="0.05" value="${this.params.pulseIntensity}">
      ...
      <div class="sf-side-panel" data-panel-content="bloom">
        <div class="sf-card">
          <h4>Bloom Effect</h4>
  ...
  <span>E</span>
    ...
    <div class="sf-side-popout" id="sf-side-popout">
      <div class="sf-side-panel" data-panel-content="shape">
        <div class="sf-card">
          ...
          <div class="sf-row">
            <div class="sf-label">Morph Duration</div>
            <div class="sf-range-wrap">
              <input class="sf-range" id="sf-md" type="range" min="0.5" max="5" step="0.1" value="${this.params.morphDuration}">
              <div class="sf-val" id="sf-md-val">${this.params.morphDuration.toFixed(2)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="sf-side-panel" data-panel-content="bloom">
        ...
        const sideIcons = el.querySelectorAll(".sf-side-icon[data-panel]");
        const sidePanels = el.querySelectorAll(".sf-side-panel");
        const popout = el.querySelector("#sf-side-popout");
        ...
        sideIcons.forEach((icon) => {
          ...
          icon.addEventListener("click", (e) => {
            ...
            const isAlreadyActive = icon.classList.contains(
              "sf-side-icon--active",
            );

            sideIcons.forEach((btn) =>
              btn.classList.remove("sf-side-icon--active"),
            );
            sidePanels.forEach((panel) =>
              panel.classList.remove("sf-side-panel--active"),
            );

            if (isAlreadyActive) {
              popout?.classList.remove("sf-side-popout--open");
              return;
            }

            icon.classList.add("sf-side-icon--active");
            popout?.classList.add("sf-side-popout--open");

            const panel = el.querySelector(`[data-panel-content="${target}"]`);
            panel?.classList.add("sf-side-panel--active");
          });
        });
        ...
        this._sidebarOutsideHandler = (event) => {
          const sidebar = el.querySelector(".sf-sidebar-glass");
          const activeCard = el.querySelector(
            ".sf-side-panel--active .sf-card",
          );

          const clickedInsideSidebar = sidebar?.contains(event.target);
          const clickedInsideActiveCard = activeCard?.contains(event.target);

          // Stay open only when clicking the icon bar OR the actual open card.
          // Clicking blank popout space should close it.
          if (clickedInsideSidebar || clickedInsideActiveCard) {
            return;
          }
          ...
          tabs.forEach((tab) => {
            tab.addEventListener("click", (e) => {
              ...
              Object.entries(panels).forEach(([key, p]) => {
                if (!p) return;
                p.classList.toggle("sf-mobile-panel--hidden", key !== target);
              });
            });
          });
        ...
        linkRange("#sf-morph", "#sf-morph-val", (v) => {
          this.params.morphDuration = v;
        });
        linkRange("#sf-ps", "#sf-ps-val", (v) => {
          this.params.pulseSpeed = v;
          if (this.wireframeMesh)
            this.wireframeMesh.material.uniforms.pulseSpeed.value = v;
        });
        linkRange("#sf-pi", "#sf-pi-val", (v) => {
          this.params.pulseIntensity = v;
          if (this.wireframeMesh)
            this.wireframeMesh.material.uniforms.pulseIntensity.value = v;
        });
        ...
        if (mwToggle) {
          mwToggle.addEventListener("change", (e) => {
            ...
            if (this.wireframeMesh)
              this.wireframeMesh.material.uniforms.multiWave.value = this.params
                .multiWave
                ? 1.0
                : 0.0;
          });
        ...
        if (this.isMorphing) {
          ...
          if (morphProgress >= 1.0) {
            ...
            if (this.guiPresetController)
              this.guiPresetController.updateDisplay();
          } else {
            this.updateWireframeGeometry();
          }
        }
      ...
      dispose() {
        ...
        if (this._sidebarOutsideHandler) {
          document.removeEventListener(
            "pointerdown",
            this._sidebarOutsideHandler,
            true,
          );
          this._sidebarOutsideHandler = null;
        }
